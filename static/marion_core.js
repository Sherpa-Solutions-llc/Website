...
function drawPath(pathArray) {
    ...
    // controls.target.set(startNode.x, startNode.y, startNode.z); // Disabled to prevent snapping
...
function initGPSMarkers(startNode, endNode) {
    ...
    redSphere.position.set(0, 16, 0); // Elevated above the node
    // Base Ring
    ...
    redGroup.position.set(startNode.x, startNode.y + 4, startNode.z);
    ...
    walkerSphere.position.set(startNode.x, startNode.y + 10, startNode.z);
    ...
    const blueMat = new THREE.MeshPhongMaterial({ color: 0x2299ff });
    const blueSphere = new THREE.Mesh(blueGeo, blueMat);
    blueSphere.position.set(0, 16, 0); // Elevated above the node
    ...
    blueGroup.position.set(endNode.x, endNode.y + 4, endNode.z);
...
function simulateRoute(pathArray) {
    ...
    function animateStep() {
        ...
        if (progress >= 1.0) {
            ...
            // Camera follows user
            // Disabled: Let the user control the map, don't force-pan it
            /*
            if (controls) {
...
function switchTo2D() {
    ...
    if (camera && controls) {
        ...
        camera.position.set(-30, 550, 60);
        controls.target.set(-30, (currentFloor - 1) * 40, 60);
        camera.lookAt(-30, (currentFloor - 1) * 40, 60);
...
function updateDirectionsUI(pathArray, nearestParking) {
    const list = document.getElementById('route-steps');
    ...
    startLi.innerHTML = `<i class="fa-solid fa-play"></i> Start at <strong>${startNode.label}</strong>`;
    list.appendChild(startLi);

    for (let i = 0; i < pathArray.length - 1; i++) {
        const current = MAP_DATA.nodes[pathArray[i]];
        const next = MAP_DATA.nodes[pathArray[i+1]];
        
        let dx = next.x - current.x;
        let dz = next.z - current.z;
        let dist = Math.sqrt(dx*dx + dz*dz) * 3; // Approx calculation for ft 
        
        let currentHeading = Math.atan2(dz, dx);
        
        if (lastHeading !== null) {
            let turnAngle = currentHeading - lastHeading;
            while (turnAngle > Math.PI) turnAngle -= 2 * Math.PI;
            while (turnAngle < -Math.PI) turnAngle += 2 * Math.PI;
            
            if (Math.abs(turnAngle) > 0.5) {
                if (continuousDistance > 0) {
                    const walkLi = document.createElement('li');
                    walkLi.innerHTML = `<i class="fa-solid fa-arrow-up"></i> Go forward ${Math.round(continuousDistance)} ft`;
                    list.appendChild(walkLi);
                    continuousDistance = 0;
                }
                
                let turnDirection = "Turn right";
                let icon = "fa-share";
                if (turnAngle < 0) {
                ...
                turnLi.innerHTML = `<i class="fa-solid ${icon}"></i> ${turnDirection}`;
                list.appendChild(turnLi);
            }
        }
        
        continuousDistance += dist;
        lastHeading = currentHeading;
        
        if (next.type === 'elevator' || next.type === 'stairs') {
            if (continuousDistance > 0) {
                const walkLi = document.createElement('li');
                walkLi.innerHTML = `<i class="fa-solid fa-arrow-up"></i> Go forward ${Math.round(continuousDistance)} ft`;
                list.appendChild(walkLi);
            ...
            transLi.innerHTML = `<i class="fa-solid fa-${next.type}"></i> Take ${next.type} to Floor ${next.floor}`;
            list.appendChild(transLi);
        }
    }
    ...
    const endNode = MAP_DATA.nodes[pathArray[pathArray.length - 1]];
    const endLi = document.createElement('li');
    endLi.innerHTML = `<i class="fa-solid fa-flag-checkered"></i> Arrive at <strong>${endNode.label}</strong>`;
...
function animate() {
    ...
    if (parkingHighlight && parkingHighlight.visible) {
      ...
      if (pathLine && gpsMarkerMeshes.walker) {
          // Traverse pathLine children to find the active line segment
          let activeLine = null;
          pathLine.children.forEach(line => {
              if (line.visible) activeLine = line;
          });

          if (activeLine) {
              const points = activeLine.geometry.attributes.position.array;
              const totalPoints = points.length / 3;
              if (totalPoints > 1) {
                  const speed = 0.005;
                  let progress = (Date.now() * speed) % (totalPoints - 1);
                  let idx = Math.floor(progress);
                  let t = progress - idx;
                  
                  let x = points[idx * 3] + (points[(idx + 1) * 3] - points[idx * 3]) * t;
                  let y = points[idx * 3 + 1] + (points[(idx + 1) * 3 + 1] - points[idx * 3 + 1]) * t;
                  let z = points[idx * 3 + 2] + (points[(idx + 1) * 3 + 2] - points[idx * 3 + 2]) * t;
                  
                  gpsMarkerMeshes.walker.position.set(x, y + 6, z);
              }
          }
      }
